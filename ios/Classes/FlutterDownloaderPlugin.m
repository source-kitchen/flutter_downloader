...
- (void)cancelAllTasks {
    ...
    [[self currentSession] getTasksWithCompletionHandler:^(NSArray<NSURLSessionDataTask *> *data, NSArray<NSURLSessionUploadTask *> *uploads, NSArray<NSURLSessionDownloadTask *> *downloads) {
        for (NSURLSessionDownloadTask *download in downloads) {
            [weakSelf cancel:download ofSession:[weakSelf currentSession]];
        };
    }];
    [[self wifiSession] getTasksWithCompletionHandler:^(NSArray<NSURLSessionDataTask *> *data, NSArray<NSURLSessionUploadTask *> *uploads, NSArray<NSURLSessionDownloadTask *> *downloads) {
        for (NSURLSessionDownloadTask *download in downloads) {
            [weakSelf cancel:download ofSession:[weakSelf wifiSession]];
        };
    }];
}
    ...
    if (taskDict != nil) {
        ...
        if ([status intValue] == STATUS_PAUSED) {
            ...
            if (resumeData != nil) {
                ...
                NSString *newTaskId = [self identifierForTask:task ofSession: (wifiOnly ? self.wifiSession : self.currentSession)];
                [task resume];
